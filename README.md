...
|---|---|---|---|
| regexPattern | RegExp | /@{0,1}import\s+["']\s*__(.*)__\s*["'];{0,1}/gi | The regular expression pattern is used to place the import statements. Note that the dark area in the default pattern is required representing the path.<br>For example, the regex for (hello <./path/here>) is (/hello\s+<**(.*)**>/gi). |
| encoding | ascii, utf8, utf-8, utf16le,<br>ucs2, base64, base64url,<br>latin1, binary, hex | utf-8 | The encoding to be used for buffering and streaming. |
